              ...
              <div class="x_panel">
                ...
                <div class="x_content">
                  ...
                  <input type="hidden" id="class_id" name="class_id" value="<?php if ($class){ echo $class->id; }else{?>0<?php }?>" />
                    ...
                    <div class="item form-group">
                      <label class="control-label col-md-2 col-sm-2 col-xs-12" for="field_one">Band
                      </label>
                      <div class="col-md-6 col-sm-6 col-xs-12">
                        <input type="text" id="band" name="band" required="required" value="<?php if($class){echo $class->band;}?>"  class="form-control col-md-7 col-xs-12">

                      </div>
                    </div>

                    <div class="item form-group">
                      <label class="control-label col-md-2 col-sm-2 col-xs-12" for="field_one">Style
                      </label>
                      <div class="col-md-6 col-sm-6 col-xs-12">
                        <input type="text" id="style" name="style" required="required" value="<?php if($class){echo $class->style;}?>"  class="form-control col-md-7 col-xs-12">
                    ...
                    <div class="item form-group">
                      <label class="control-label col-md-2 col-sm-2 col-xs-12" for="field_one">原创
                      </label>
                      <div class="col-md-6 col-sm-6 col-xs-12">
                        <input type="radio" name="original" value="0" <?php if($class){ if($class->original == 0){?>checked <?php }}else{?> checked <?php }?>>否
                        <input type="radio" name="original" value="1" <?php if($class){ if($class->original == 1){?>checked <?php }} ?>>是
                      </div>
                    </div>
    ...
    $('form').submit(function(e) {
      ...
      if (submit){
        var submitData={
            ...
            band:$("input[name='band']",$form).val(),
            style:$("input[name='style']",$form).val(),
            link:$("input[name='link']",$form).val(),
            original:$("input[name='original']:checked",$form).val(),
            id:$("input[name='class_id']",$form).val(),
        };
         $.post('/admin/suggest_song_add',submitData,function(data){

            if(data.success=="yes")
            {
               show_stack_modal('success',data.msg);
               common_go_next("/admin/suggest_song_list");
            }else
            {
